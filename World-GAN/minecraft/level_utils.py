...
def read_level(opt: Config):
    ...
    # Format token list for logging - show actual names
    token_names = [str(t) if isinstance(t, str) else str(t) for t in opt.token_list]
    logger.info("Tokens in level {}", token_names)
...
def read_level_from_file(input_dir, input_name, coords, block2repr, repr_type, debug=False):
    ...
    # Read PyAnvil format
    # coords is ((y0,yend), (z0,zend), (x0,xend)) but get_block expects (x, y, z)
    failed_blocks = 0
    total_blocks = (coords[0][1] - coords[0][0]) * (coords[1][1] - coords[1][0]) * (coords[2][1] - coords[2][0])
    processed = 0
    print(f"Starting block extraction: {total_blocks} total blocks...")
    with World(input_name, input_dir, debug=debug) as wrld:
        for y in range(coords[0][0], coords[0][1]):
            for z in range(coords[1][0], coords[1][1]):
                for x in range(coords[2][0], coords[2][1]):
                    try:
                        block = wrld.get_block((x, y, z))
                        # Get block name as string - handle both string and object returns
                        block_state = block.get_state()
                        b_name = str(block_state.name) if hasattr(block_state, 'name') else str(block_state)
                        # Extract just the block ID if it's a full path like 'minecraft:stone'
                        if ':' in b_name:
                            b_name = b_name.split(':')[1]
                        
                        if repr_type == "block2vec":
                            level[0, :, y - coords[0][0], z - coords[1][0], x - coords[2][0]] = block2repr[b_name]
                            ...
                            if b_name not in uniques:
                                ...
                                print(f"  Found new block type: {b_name} (total types: {len(uniques)})")
                            level[y - coords[0][0], z - coords[1][0], x - coords[2][0]] = uniques.index(b_name)
                    except Exception as e:
                        # If block read fails, skip it and treat as air
                        failed_blocks += 1
                        if failed_blocks <= 5:
                            print(f"Warning: Failed to read block at ({x}, {y}, {z}): {e}")
                    
                    processed += 1
                    if processed % 100000 == 0:
                        print(f"  Progress: {processed}/{total_blocks} ({100*processed/total_blocks:.1f}%) - {len(uniques)} block types found")
    
    if failed_blocks > 0:
        print(f"Total failed block reads: {failed_blocks}/{(coords[0][1]-coords[0][0])*(coords[1][1]-coords[1][0])*(coords[2][1]-coords[2][0])}")
...
def clear_empty_world(worlds_folder, empty_world_name='Curr_Empty_World'):
    ...
    if os.path.exists(dst):
        shutil.rmtree(dst)
    # If source exists, copy it; otherwise create an empty destination directory
